...
class RelatorioSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not validate_qualificacao_profissional(data["qualificacao_profissional"]):
            raise serializers.ValidationError(
                {
        ...
        if not validate_presfauna(data["presfauna"]):
            raise serializers.ValidationError(
                {
                    "presfauna": "Presença de fauna inválida. As opções são: AK1 Desprezível ou AK2 Prejudicial"
                }
            )
